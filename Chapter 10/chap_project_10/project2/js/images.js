...
//Loop through images array and using the appropriate jQuery DOM
//methods, add the appropriate <img> tags to the supplied <ul class="gallery">
//element. The image filenames are contained in the path property of each 
//image object. Set the alt attribute of each <img> to the title property 
//of the image object.
...
$(function () {
	...
	for(let i = 0; i<images.length; i++) {
		...
		img.on("mouseenter", function (e) {
			$(this).addClass("gray");
			var prevDiv = $("<div>").attr("id", "preview");
			var prevImg = $("<img>").attr("src", "images/medium/" + images[i].path);
			var p = $("<p>");
			p.append(images[i].title);
			p.append($("<br>"));
			var em = $("<em>");
			em.append(images[i].city + ", " + images[i].country + " ");
			em.append("[" + images[i].taken + "]");
			p.append(em);
			prevDiv.append(prevImg);
			prevDiv.append(p);
			
			prevDiv.css("top", (e.pageY + 30) + "px");
			prevDiv.css("left", (e.pageX + 30) + "px");
		...
		img.on("mousemove", function (e) {
			$("#preview").css("top", (e.pageY + 20) + "px");
			$("#preview").css("left", (e.pageX + 20) + "px");
		});
